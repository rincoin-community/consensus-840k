...
Build: development configuration (see [`build-manifest.json`](build-manifest.json)), the sources of the
branch tip listed there. Date: 2026-09-26.
...
## Test cases added for the transition (24)
...
| `s6b_miner_tests` | 3 | block assembly with a mempool entry signed for the other side of the transition, placed there directly because the mempool eviction otherwise never leaves one: the template is still produced, leaves the entry out, and the block connects; an old entry that still verifies is kept; below the transition height nothing is left out. A negative control with the safeguard removed makes `CreateNewBlock` throw, which is the failure this prevents |
