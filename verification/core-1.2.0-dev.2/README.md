...
| [`unit-test-results.md`](unit-test-results.md) | `make check`: commands, counts, the one skip, the 21 test cases added for the transition, the check against real Bitcoin Gold signatures |
| [`functional-test-results.md`](functional-test-results.md) | the functional suite on both builds: counts, baseline failures, skips, the six new tests and what each establishes, two negative controls |
...
| What | Result |
|---|---|
| Unit tests (`make check`, development configuration) | passed; 555 test cases, 1 of them a skip |
...
| Functional tests, CI allowlist (164), production configuration | 164 passed |
