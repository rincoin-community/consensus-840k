...
Source tree `7924b96b3` (branch tip `b7bc1a9ef`); builds as in [`build-manifest.json`](build-manifest.json). Date:
2026-09-21. All runs on regtest, on one Linux host, 16–20 tests in parallel.

| Run | Binaries | Result | Wall time |
|---|---|---|---|
| CI allowlist (`test/functional/ci_passing_tests.txt`, 164 tests) | production configuration | **164 passed**, 0 failed, 0 skipped | 968 s |
| CI allowlist | development configuration | **164 passed**, 0 failed, 0 skipped | 851 s |
| complete base set of `test_runner.py` (222 invocations) | development configuration | 178 passed, **42 skipped**, **2 failed** (both are baseline failures, below) | 291 s |
