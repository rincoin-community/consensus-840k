# S6/b — Coinbase Commitment Test Vector Catalog
...
[S1's catalog](../S1/commitment-test-vectors.md); this file exists so
S6/b's annex is self-contained rather than requiring a cross-reference for
its own evidence.
...
| ID | Case | Test |
| --- | --- | --- |
| FC-P01 | Exact, correct commitment for the S6/b tuple (`branch_id`/`fork_no`/`scenario_id=3`) | `build_and_parse_round_trip` |
...
Both open items are pre-existing, inherited from S1's own unresolved open
items (also present in S5/b's annex), not new to this scenario — carried
over rather than independently rediscovered. Closing them once closes them
for all three scenario branches.
